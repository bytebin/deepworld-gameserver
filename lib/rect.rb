...
class Rect
  ...
  def center
    Vector2.new(self.centerx, self.centery)
  end
  ...
  def collide_rect?(other, inclusive = true)
    nself = normalize
    ...
    ( nself.left.between?(other.left, other.right - (inclusive ? 0 : 1)) or
      other.left.between?(nself.left, nself.right - (inclusive ? 0 : 1)) ) and
      ( nself.top.between?(other.top, other.bottom - (inclusive ? 0 : 1)) or
        other.top.between?(nself.top, nself.bottom - (inclusive ? 0 : 1)) )
  end
  ...
  def contains?(coordinate)
    ...
    coord[0] >= rect.left && coord[0] <= rect.right && coord[1] >= rect.top && coord[1] <= rect.bottom
  end
